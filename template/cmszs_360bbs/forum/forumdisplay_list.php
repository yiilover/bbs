...
<div id="threadlist" class="tl bm bmw  noborder radius5 block-list"{if $_G['uid']} style="position: relative;"{/if}>
<!--{if ($_G['forum']['threadtypes'] && $_G['forum']['threadtypes']['listable']) || count($_G['forum']['threadsorts']['types']) > 0}-->
<!--{if $leftside}-->
<style>
#thread_types{width: 654px;}
</style>
<!--{else}-->
<style>
#thread_types{width: 883px;}
</style>
<!--{/if}-->
					<ul id="thread_types" class="ttp cl  radius4">
						<!--{hook/forumdisplay_threadtype_inner}-->
						<li id="ttp_all" {if !$_GET['typeid'] && !$_GET['sortid']}class="xw1 a"{/if}><a href="forum.php?mod=forumdisplay&fid=$_G[fid]{if $_G['forum']['threadsorts']['defaultshow']}&filter=sortall&sortall=1{/if}{if $_GET['archiveid']}&archiveid={$_GET['archiveid']}{/if}">{lang forum_viewall}</a><span class="type-pipe"></span></li>
						<!--{if $_G['forum']['threadtypes']}-->
							<!--{loop $_G['forum']['threadtypes']['types'] $id $name}-->
								...
								<!--{else}-->
								<li><a href="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=typeid&typeid=$id$forumdisplayadd[typeid]{if $_GET['archiveid']}&archiveid={$_GET['archiveid']}{/if}"><!--{if $_G[forum][threadtypes][icons][$id] && $_G['forum']['threadtypes']['prefix'] == 2}--><img class="vm" src="$_G[forum][threadtypes][icons][$id]" alt="" /> <!--{/if}-->$name<!--{if $showthreadclasscount[typeid][$id]}--><!--<span class="xg1 num">$showthreadclasscount[typeid][$id]</span>--><!--{/if}--></a><span class="type-pipe"></span></li>
								<!--{/if}-->
							<!--{/loop}-->
						<!--{/if}-->
						...
						<!--{if $_G['forum']['threadsorts']}-->
							...
							<!--{loop $_G['forum']['threadsorts']['types'] $id $name}-->
								<!--{if $_GET['sortid'] == $id}-->
								<li class="xw1 a"><a href="forum.php?mod=forumdisplay&fid=$_G[fid]{if $_GET['typeid']}&filter=typeid&typeid=$_GET['typeid']{/if}{if $_GET['archiveid']}&archiveid={$_GET['archiveid']}{/if}">$name<!--{if $showthreadclasscount[sortid][$id]}--><!--<span class="xg1 num">$showthreadclasscount[sortid][$id]</span>--><!--{/if}--></a><span class="type-pipe"></span></li>
								<!--{else}-->
