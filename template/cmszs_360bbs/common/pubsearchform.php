...
<div id="search_bar" class="{if $_G['setting']['srchhotkeywords'] && count($_G['setting']['srchhotkeywords']) > 5}scbar_narrow {/if}cl">
	<form id="scbar_form" method="{if $_G[fid] && !empty($searchparams[url])}get{else}post{/if}" autocomplete="off" onsubmit="searchFocus($('scbar_txt'))" action="{if $_G[fid] && !empty($searchparams[url])}$searchparams[url]{else}search.php?searchsubmit=yes{/if}" target="_blank">
		...
		<!--{/if}-->
		<input type="text" name="srchtxt" id="scbar_txt" autocomplete="off" class="xg1 radius5" placeholder=" {lang enter_content}"/>
        
<input type="submit" name="searchsubmit" class="search-btn" value=" " autocomplete="off">
	</form>
</div>
<ul id="scbar_type_menu" style="display: none;"><!--{echo implode('', $slist);}--></ul>
